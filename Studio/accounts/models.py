...
class UserManager(BaseUserManager):
    def create_user(self, email, f_name, phone, create, password):
        user = self.model(email=self.normalize_email(email), f_name=f_name,  phone=phone,
                          create=create, )
        user.set_password(password)
        user.save(using=self._db)
        return user

    def create_superuser(self, email, f_name, phone, create, password):
        user = self.create_user(email, f_name,phone, create, password)
        user.is_admin = True
